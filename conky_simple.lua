...
function conky_main()
  ...
  -- TIME
  cairo_set_font_size(cr, 0.040 * WINDOW_WIDTH)
  cairo_move_to(cr, 0.80 * WINDOW_WIDTH, 0.150 * WINDOW_HEIGHT)
  cairo_show_text(cr, conky_parse("${time %H:%M}"))
  cairo_stroke(cr)
  
  -- DATE
  cairo_set_font_size(cr, 0.010 * WINDOW_WIDTH)
  cairo_move_to(cr, 0.80 * WINDOW_WIDTH, 0.184 * WINDOW_HEIGHT)
  ...
  local cx,cy = 0.660 * WINDOW_WIDTH, 0.15 * WINDOW_HEIGHT
  local radius = 0.035 * WINDOW_WIDTH
  ...
  -- for line = 1,table.getn(processes) do
    cairo_move_to(cr, 0.830 * WINDOW_WIDTH, 0.20 * WINDOW_HEIGHT + line * 0.014 * WINDOW_HEIGHT)
  ...
  local cx,cy = 0.715 * WINDOW_WIDTH, 0.090 * WINDOW_HEIGHT
  local grid_width = 0.012 * WINDOW_WIDTH
  for i = 1,40 do
...
function conky_fs_main()
  ...
  local offset = 0.295 * 5120
  local gap = 0.053 * WINDOW_WIDTH

  draw_volume("     /", tonumber(conky_parse("${fs_used_perc /}")) , offset)
  draw_volume("   media", tonumber(conky_parse("${fs_used_perc /dev/sda1}")) , offset - gap)
--
--  local dropbox_perc = 0
--  local file = io.popen("python3 ~/.config/conky/conky-simple/dropbox/get_quota.py")
...
--
--  local drive_perc = 0
--  local file = io.popen("python3 ~/.config/conky/conky-simple/drive/get_quota.py")
--  for line in file:lines() do
